...
export class Room{
 ...
 async load(){if(this.loaded)return;this.loaded=true;/* Active rooms live only while their WebSockets are alive. Do not resurrect stale room codes after a Durable Object restart. */await this.ctx.storage.delete('roomConfigs')}
 ...
 assign(r,p,preferred=p.preferredTeam){
   ...
   // Sunucu spawn noktaları istemcideki gerçek harita üsleriyle birebir aynı olmalı.
   // T: güneybatı başlangıcı, CT: kuzey başlangıç avlusu.
   const b=team==='T'?{x:-40,z:12,yaw:0}:{x:4,z:-60,yaw:Math.PI};
 ...
 async handle(id,raw){let m;try{m=JSON.parse(raw)}catch{const p=this.players.get(id);if(p)this.send(p.ws,{type:'error',message:'Geçersiz ağ mesajı.'});return}const p=this.players.get(id);if(!p)return;try{
 ...
 }catch(err){this.send(p.ws,{type:'error',message:'Sunucu oda işlemini işlerken hata oluştu.'});}
 }
