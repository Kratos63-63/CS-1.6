...
function spawn(p){const base=p.team==='T'?{x:-40,z:12,yaw:0}:{x:-16,z:-60,yaw:Math.PI};const slots=[[0,0],[-2,0],[2,0],[0,-2],[0,2],[-2,-2],[2,-2],[-2,2]];const [dx,dz]=slots[p.slot%slots.length];p.x=base.x+dx;p.z=base.z+dz;p.y=1.7;p.yaw=base.yaw;p.pitch=0;}
