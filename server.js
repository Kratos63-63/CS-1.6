...
function spawn(p){const base=p.team==='T'?{x:-20,z:32,yaw:0}:{x:4,z:-40,yaw:Math.PI};const n=p.slot%8;const a=(n%4)-1.5,b=Math.floor(n/4)-.5;p.x=base.x+a*1.25;p.z=base.z+b*1.25;p.y=1.7;p.yaw=base.yaw;p.pitch=0;}
...
function makePlayer(ws){return{id:crypto.randomUUID(),ws,room:null,team:'T',slot:0,x:-20,y:1.7,z:32,yaw:0,pitch:0,health:100,alive:true,kills:0,deaths:0,weapon:'ak',ammo:Object.fromEntries(Object.entries(WEAPONS).map(([k,w])=>[k,w.magazine])),reserve:Object.fromEntries(Object.entries(WEAPONS).map(([k,w])=>[k,w.reserve])),input:{forward:0,side:0,run:false,yaw:0,pitch:0},lastShot:0,respawnAt:0};}
